...
- The same three optima are constructed with no schedule hint: A in 209 seconds, B in 155 seconds, and C in 85 seconds under 300-second budgets. A deterministic one-worker portfolio also reproduces every score. Search preserves the best safe incumbent and limits each solve round to three seconds so an invalid relaxation cannot consume the whole budget or overwrite a valid output.
