...
- Adding one independent three-unit, two-week-deadline activity to the frozen 324-activity dense case creates proved A/C=`7.0` and B=`10.0` optima. The fixed policy reaches strict-clean A/C=`7.0` but fails B because the structural constructor supplies 324/325 activities and omits the ECLO-required activity; the unrestricted B fallback expands to 13.18 million variables and returns `UNKNOWN`. This failure is preserved before repair.
